...
resource "aci_bridge_domain" "bdLocalName" {
  ...
  relation_fv_rs_bd_to_out = [ module.l3out.vrf_global_l3out2.l3out_dn ]
  depends_on = [
    module.l3out
  ]
}
...
# L3out
module "l3out" {
  for_each = toset([ "vrf_global_l3out1", "vrf_global_l3out2" ])

  source = "./modules/l3out"
  tenant_dn = aci_tenant.tenantLocalName.id
  vrf_dn = aci_vrf.vrfLocalName.id
  l3out_name = each.value
}

# SVI
resource "aci_rest" "restLocalName" {
  path       = "/api/node/mo/uni/tn-vanilla/out-vrf_global_l3out1/lnodep-global_vrf_l3out_nodeProfile/lifp-global_vrf_l3out_interfaceProfile/rspathL3OutAtt-[topology/pod-1/paths-112/pathep-[eth1/2]].json"
